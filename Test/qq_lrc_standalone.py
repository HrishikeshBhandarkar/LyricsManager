...
def _qq_des_ecb(data: bytes, key: bytes, mode: int) -> bytes:
    """mode: 0 = encrypt, 1 = decrypt. Uses QQ's buggy DES, ECB, in place."""
    global _DES_LIB
    if _DES_LIB is None:
        _DES_LIB = _build_or_load_des_lib()
    buf = ctypes.create_string_buffer(data, len(data))
    _DES_LIB.qq_des_ecb(buf, len(data), key, mode)
    return buf.raw


def qq_des_decrypt(data: bytes) -> bytes:
    """
    ...
    """
    step1 = _qq_des_ecb(data, QRC_KEY1, mode=1)   # decrypt
    step2 = _qq_des_ecb(step1, QRC_KEY2, mode=0)  # encrypt
    step3 = _qq_des_ecb(step2, QRC_KEY3, mode=1)  # decrypt
    return step3


# --------------------------------------------------------------------------
# QQ Music API calls (unchanged from the original script)
# --------------------------------------------------------------------------
...
def fetch_qrc_raw(songmid: str, songid: int, debug: bool = False):
    ...
    resp = requests.get(QRC_URL, params=params, headers=HEADERS, timeout=10)
...
def fetch_plain_lrc(songmid: str) -> str:
    ...
    resp = requests.get(LYRIC_FALLBACK_URL, params=params, headers=HEADERS, timeout=10)
    resp.raise_for_status()
    try:
        data = resp.json()
    except ValueError:
...
def main():
    ...
    lrc = fetch_plain_lrc(result["songmid"])
    if not lrc:
