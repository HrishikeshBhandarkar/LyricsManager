...
Pipeline:
  ...
  eLRC line examples this produces:
...
def search_lyric_candidates(name: str, artist: str, duration_ms: int):
    resp = requests.get(
        LYRIC_SEARCH_URL,
        params={"ver": 1, "man": "yes", "client": "pc", "keyword": f"{name} - {artist}", "duration": duration_ms},
        headers=HEADERS,
        timeout=10,
    )
    resp.raise_for_status()
    ...
    if not candidates:
        raise RuntimeError("No lyric candidates found")
    return candidates
...
def to_elrc(parsed_lines, alternate_voices: bool = False) -> str:
    ...
    for line in parsed_lines:
        ...
        word_parts = []
        for text, start_ms, _dur_ms in words:
            clean_text = strip_parens(text) if bg else text
            if not clean_text.strip():
                continue
            word_parts.append(f"{clean_text}{{{ms_to_tag(start_ms)}}}")
...
def main():
    ...
    try:
        ...
        print(f"Found: {song.get('trackName')} - {song.get('artistName')} "
              f"({duration_ms/1000:.0f}s)", file=sys.stderr)

        candidates = search_lyric_candidates(args.name, args.artist, duration_ms)
        picked = candidates[0]
